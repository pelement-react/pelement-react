...
const Message: React.ForwardRefRenderFunction<HTMLDivElement, MessageProps> = (
  ...
  return (
    <div
      ref={ref}
      id="message_11"
      className="el-message el-message--info"
      role="alert"
    ...
    >
      ...
      <p className="el-message__content">This is a message.</p>
    </div>
  )
}
...
const message = {
  success: (message: string | React.ReactNode) => {
    ...
    messages.forEach((item) => {
      verticalOffset += (item.el?.querySelector('.el-message').offsetHeight || 0) + 16
    })
    messages.push({
      id: id,
      message: message,
    ...
    root = createRoot(el)
    root.render(<MessageComponent offset={verticalOffset} />)
  }
}
